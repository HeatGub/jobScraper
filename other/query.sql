...
SELECT  name FROM sqlite_master --table
WHERE type='table' --tejbyl
ORDER BY name; -- EXISTING TABLES LIST
...
-- SELECT (JULIANDAY(datetimeLast) - JULIANDAY(datetimeFirst)) FROM test1 ORDER BY (JULIANDAY(datetimeLast) - JULIANDAY(datetimeFirst)) * 24 * 60 DESC;